...
uniform samplerCUBE _GlobalEffectRTCubeMap;
...
Varyings vert(Attributes input)
{
    ...
    //create local uv
    float3 uv = worldPosition - _Position;
    uv = uv / (_OrthographicCamSize * 2);
    uv += 0.5;
    
    // Effects RenderTexture Reading
    float4 RTEffect = texCUBElod(_GlobalEffectRTCubeMap, float4(input.normal,0));
    //RTEffect.w = 1;
    // smoothstep to prevent bleeding
    //RTEffect *= smoothstep(0.99, 0.9, uv.x) * smoothstep(0.99, 0.9, 1 - uv.x) * smoothstep(0.99, 0.9, 1 - uv.y) * smoothstep(0.99, 0.9, 1 - uv.z);
    //RTEffect *= smoothstep(0.99, 0.9, uv.y) * smoothstep(0.99, 0.9, 1 - uv.y);
    //RTEffect *= smoothstep(0.99, 0.9, uv.z) * smoothstep(0.99, 0.9, 1 - uv.z);
    ...
    input.vertex.xyz += SafeNormalize(input.normal) * saturate(( _SnowHeight) + (SnowNoise * _NoiseWeight)) * saturate(1-(RTEffect.r * _SnowDepth));
